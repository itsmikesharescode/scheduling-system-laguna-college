...
<main class="flex flex-col gap-10 px-[2rem] py-10">
  <h1 class="mt-5 text-center text-5xl font-bold" in:fly={{ x: -100 }}>TEACHER DASHBOARD</h1>
  ...
  <div class="container grid grid-cols-2 gap-5">
    <section class="grid grid-rows-[3fr_2fr] gap-2.5">
      ...
      >
        ...
        <Button href="/teacher/schedule" variant="secondary" size="sm">View Schedule</Button>
      </section>
      <section
      ...
      <ScrollArea class="b max-h-[30dvh]">
        <div class="flex flex-col gap-2.5">
          {#each todaySubjects as subject}
            <a href="/teacher/schedule" class="transition-all hover:underline">{subject.name}</a>
          {/each}
        </div>
      </ScrollArea>
    </section>
